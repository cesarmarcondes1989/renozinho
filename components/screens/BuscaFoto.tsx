    ...
    <div style={css("display:flex;flex-direction:column;min-height:100%;animation:ihslide .26s ease both")}>
      ...
      {estado === "camera" && (
        <div style={css("padding:18px;display:flex;flex-direction:column;gap:16px;flex:1")}>
          ...
          <div style={css(`position:relative;flex:1;min-height:280px;border-radius:22px;background:${t.bgCard};border:1px solid ${t.border};display:flex;align-items:center;justify-content:center;overflow:hidden`)}>
            <span style={css(`position:absolute;inset:34px;border-radius:16px;border:1.5px dashed ${t.accentDashedBorder}`)} />
            <span style={css(`font-size:11.5px;font-weight:700;color:${t.textTertiary};letter-spacing:.04em`)}>VISOR DA CÂMERA</span>
          </div>
          <button
            onClick={() => fileInputRef.current?.click()}
            style={css(
              `display:flex;align-items:center;justify-content:center;gap:10px;background:${t.accent};color:${t.accentText};border:none;border-radius:16px;padding:17px;font-size:15px;font-weight:800;cursor:pointer`
            )}
          >
          ...
          <button
            onClick={() => fileInputRef.current?.click()}
            style={css(`background:${t.bgCard};border:1px solid ${t.borderStrong};color:${t.textBright};border-radius:14px;padding:14px;font-size:12.5px;font-weight:700;cursor:pointer`)}
          >
            Escolher da galeria
          </button>
        </div>
      ...
      {estado === "analisando" && (
        <div style={css("padding:70px 26px;display:flex;flex-direction:column;align-items:center;gap:20px;text-align:center")}>
          <div style={css(`width:60px;height:60px;border-radius:999px;border:3px solid ${t.border};border-top-color:${t.accent};animation:ihspin .9s linear infinite`)} />
          <div style={css("font-size:15px;font-weight:800;letter-spacing:-.02em")}>Comparando com seu estoque…</div>
          <div style={css(`font-size:12.5px;font-weight:500;color:${t.textSecondary};max-width:270px;line-height:1.6`)}>
      ...
      {estado === "resultado" && (
        <div style={css("padding:18px;display:flex;flex-direction:column;gap:16px")}>
          <div style={css(`display:flex;gap:12px;align-items:center;background:${t.bgCard};border:1px solid ${t.border};border-radius:18px;padding:12px`)}>
            {fotoUrl ? (
            ...
            <span style={css("flex:1;min-width:0;display:flex;flex-direction:column;gap:3px")}>
              ...
              <span style={css(`font-size:11.5px;font-weight:500;color:${t.textSecondary};line-height:1.5`)}>{descricao}</span>
            </span>
          </div>
          ...
          <div style={css("display:flex;flex-direction:column;gap:9px")}>
            {similares.map((s) => {
              ...
              return (
                <button
                  ...
                  style={css(`display:flex;align-items:center;gap:12px;background:${t.bgCard};border:1px solid ${t.border};border-radius:18px;padding:11px;cursor:pointer;text-align:left;color:${t.textPrimary}`)}
                >
                  <Thumb p={p} size={64} radius={14} />
                  <span style={css("flex:1;min-width:0;display:flex;flex-direction:column;gap:3px")}>
                    <span style={css("display:flex;align-items:center;gap:7px")}>
                      <span style={css(`font-size:10px;font-weight:800;color:${t.textSecondary};letter-spacing:.05em;text-transform:uppercase`)}>{p.marca}</span>
                      <span
                        style={css(
                          `font-size:10.5px;font-weight:800;color:${
                            s.score > 80 ? t.accent : s.score > 65 ? "#F5C518" : t.textTertiary
                    ...
                    <span style={css(`font-size:10.5px;font-weight:500;color:${t.textTertiary};line-height:1.4`)}>{s.motivo}</span>
                    <span style={css(`font-size:11px;font-weight:700;color:${t.accent}`)}>
            ...
            {!similares.length && (
              <div style={css(`font-size:12px;font-weight:500;color:${t.textSecondary};padding:8px 2px`)}>
          ...
          <div style={css("display:flex;gap:9px")}>
            <button
              ...
              style={css(`flex:1;background:${t.bgCard};border:1px solid ${t.borderStrong};color:${t.textBright};border-radius:15px;padding:15px;font-size:13px;font-weight:700;cursor:pointer`)}
            >
              Tirar outra foto
            </button>
            <button
              ...
              style={css(`flex:1;background:${t.accent};color:${t.accentText};border:none;border-radius:15px;padding:15px;font-size:13px;font-weight:800;cursor:pointer`)}
            >
              Cadastrar como nova
            </button>
          </div>
